...
def alpaca_prompt(prompt, context=None, system=""):
    if context:
        formatted_prompt = (
            # alpaca prompt with context
            f"{DEFAULT_INTRO_CONTEXT} {system}\n\n"
            + f"### Input: {context}\n\n"
        ...
        formatted_prompt = (
            # alpaca prompt without context
            f"{DEFAULT_INTRO} {system}\n\n"
            + f"### Instruction: {prompt}\n\n"
            + "### Response:"
        )
...
def bloomz_prompt(prompt, context=None, system=""):
    if context:
        formatted_prompt = (
            # alpaca-inspired prompt with context
            f"{system} "
            + f"Input: {context}. "
            + f"{DEFAULT_INTRO_CONTEXT} "
            + f"Instruction: {prompt}? "
            + "Response:"
        )
    else:
        formatted_prompt = (
            # alpaca-inspired prompt, no context
            f"{system} "
            + f"{DEFAULT_INTRO} "
            + f"Instruction: {prompt}? "
            + "Response:"
...
template_mapping = {
    "elm-small": alpaca_prompt,
    "mpt-7b-instruct": alpaca_prompt,
    "mpt-7b-chat": alpaca_prompt,
    "mpt-30b-instruct": alpaca_prompt,
    "mpt-30b-chat": alpaca_prompt,
